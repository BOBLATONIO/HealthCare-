<x-authentication-layout title="Dashboard">
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        ...
        <div class=" w-full bg-white rounded-lg shadow  mt-5 s  max-w-sm">
            <div class="p-6 ">
                <div>
                    <h1 class="font-semibold w-full text-center mb-4 text-lg">Enter your new password</h1>
                </div>
                <form method="POST" class="space-y-6" action="{{ route('new-password') }}">
                    @csrf
                    @if(session('error'))
                    <div id="alertMessage"
                   ...
                   <div class="mb-2">
                    ...
                    <div class="flex items-center justify-between">
                        <a href="{{ route('login') }}"
                            class="text-sm font-medium text-primary-600 hover:underline">Login instead.</a>
                    </div>
                    <button type="submit"
                        class="w-full text-white bg-blue-800 bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg  px-5 py-2.5 text-center ">
                        Change Password
                    </button>
                </form>
            </div>
